...
cardio_clean$age <- floor(cardio_clean$age / 365)
```
...
```{r}
results1 = table(cardio_log_train$cardio, predictTrain > 0.4)
results1
```
...
results_test1 = table(cardio_log_test$cardio, predictTest >0.4)
results_test1
```
...
ROCRperf1 = performance(ROCRpred1, "tnr", "fnr")
...
```{r}
results_test2 = table(cardio_log_test$cardio, predictTest1 >0.4)
results_test2
```
...
ROCRperf_pcr = performance(ROCRpred_pcr, "tnr", "fnr")
...
```{r}
results_pcrtest = table(cardio_pcr_test$cardio, predictTest_pcr > 0.4)
results_pcrtest
```
...
ROCRperf_pcr1 = performance(ROCRpred_pcr1, "tnr", "fnr")
...
results_pcrtest1 = table(cardio_pcr_test$cardio, predictTest_pcr > 0.4)
results_pcrtest1
```
